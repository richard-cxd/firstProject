...
public class AlarmWebController extends BaseController{
    ...
    public List<LinkedHashMap> findmessage(String aliasId ){
    ...
    public DetailDTO alarmsetting(HttpServletRequest request, HttpServletResponse response, @RequestBody String data){
       ...
       System.out.print(data);
        ...
        if (re != null){
            returnDetailDTO.setDetail(re);
        }else {
            returnDetailDTO.setDetail("后台出错！");
        }
...
//拉取历史数据
    @RequestMapping(value="datapullsetting" ,method =RequestMethod.POST )
    public String  datapullsetting(HttpServletRequest request, HttpServletResponse response, @RequestBody String data){

        DetailDTO datailDTO = new DetailDTO(true);
        System.out.print(data);
        String re = alarmService.datapullsetting(data);
