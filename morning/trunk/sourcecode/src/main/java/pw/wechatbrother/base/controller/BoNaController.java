...
public class BoNaController {
   ...
   //告警数据上报
    ...
    public RestStatus addGJ(HttpServletResponse response ,@RequestBody Map map){
        RestStatus restStatus = new RestStatus(true);
        try {
           Map hashmap = new HashMap();
           hashmap = map;
           String jcdId = hashmap.get("jcdId").toString();
           String  DataDate = hashmap.get("DataDate").toString();
           String  level = hashmap.get("level").toString();
           String  type = hashmap.get("type").toString();
           String  msg = hashmap.get("msg").toString();
           String lastdata = hashmap.get("lastdata").toString();
            boNaService.addGJ(jcdId,DataDate,level,type,msg,lastdata);
